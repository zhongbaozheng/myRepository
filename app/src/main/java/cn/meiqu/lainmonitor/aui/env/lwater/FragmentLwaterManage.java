...
import cn.meiqu.lainmonitor.adapter.RecycleLocationManageAdapter;
 ...
 */
public class FragmentLwaterManage extends FragmentAlert implements RecycleLocationManageAdapter.OnItemClickListner {
    String action_getData = className + API.getLocationManage;
    ...
    public void requestData(String start, String end, String deviceId) {
        showProgressDialog();
        HttpGetController.getInstance().getLocationManageList(className);
    }

    public void handleData(String data) {
        ArrayList<LocationWater> temps = new Gson().fromJson(data, new TypeToken<ArrayList<LocationWater>>() {
        }.getType());
        locationWaters.clear();
        locationWaters.addAll(temps);
        adapter.notifyDataSetChanged();
    }

    public void requestLocations() {
        HttpGetController.getInstance().getLocationLocationsList(className);
    }
    ...
    public void requestAdd(String address, String location, String ipAddress, String name) {
        showProgressDialog();
        HttpGetController.getInstance().addLocationManage(address, location, ipAddress, name, className);
    }

    public void requestEdt(String id, String name) {
        showProgressDialog();
        HttpGetController.getInstance().edtLocationManage(id, name, className);
    }

    //
    public void requestDel(String id, String ip) {
        HttpGetController.getInstance().delLocationManage(id, ip, className);
    }
    ...
    public void showEdtDialog(final int position) {
        View body = LayoutInflater.from(getActivity()).inflate(R.layout.layout_locationwater_input, null);
        ...
        String title = "设备修改";
        if (position != -1) {
            final LocationWater locationWater = locationWaters.get(position);
            mEdtAddress.setText("" + locationWater.getAddress());
            mEdtLocation.setText("" + locationWater.getDeviceLocationPojo().getDlName());
            mEdtIp.setText("" + locationWater.getIpPort());
            mEdtName.setText(locationWater.getName() + "");
        } else {
            title = "设备添加";
    ...
    public void onItemDel(final int position) {
        AlertDialog alertDialog = new AlertDialog.Builder(getActivity()).setMessage("是否要删除" + locationWaters.get(position).getName() + "?").setNegativeButton("取消", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
            ...
            @Override
            public void onClick(DialogInterface dialog, int which) {
                requestDel(locationWaters.get(position).getId() + "", locationWaters.get(position).getIp() + "");
            }
        }).show();
    }
